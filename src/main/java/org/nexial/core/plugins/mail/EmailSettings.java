...
public class EmailSettings {
    ...
    public void setFailure(String config, String message) {
        if (!MapUtils.isNotEmpty(failures)) { failures = new HashMap<>(); }
        failures.put(config, message);
    }

    public void clearFailure(String config) { if (MapUtils.isNotEmpty(failures)) { failures.remove(config); } }
    ...
    public String toString() {
        return new ToStringBuilder(this).append("toRecipients", toRecipients)
    ...
    public String displayFailures() {
        if (MapUtils.isEmpty(failures)) { return EMPTY; }

        StringBuilder messages = new StringBuilder(EMPTY);
        for (String key : failures.keySet()) { messages.append(failures.get(key)).append("\n"); }
