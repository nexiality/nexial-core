...
class TableHelper(private val webCommand: WebCommand) {
	...
	fun saveTableAsCsv(locator: String, nextPageLocator: String, file: String): StepResult {
		...
		// changed from 'val' to 'var' because we might need to re-fetch TABLE element after pagination
		var table = webCommand.toElement(locator)
		...
		while (true) {
			...
			try {
				if (!table.isEnabled) table = webCommand.toElement(locator)
