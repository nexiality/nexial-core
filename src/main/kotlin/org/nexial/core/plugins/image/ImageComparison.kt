...
class ImageComparison(expected: BufferedImage, actual: BufferedImage) {
    ...
    fun compareImages(color: Color?): Float {
        ...
        val matchedPercent: Float = (if (result!!.imageComparisonState == MATCH) 1f else 1 - result!!.differencePercent)
        matchPercent = IMAGE_PERCENT_FORMAT.format((matchedPercent * 100).toDouble()).toFloat()
        return matchPercent
    }
