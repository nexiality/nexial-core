...
object JavaUIMapping {
    ...
    private const val MSG_MISSING_JUBULA_HOME = "unable to resolve Jubula installation directory via environment " +
                                                "variable $envJubulaHome nor via commandline option (-h)"
    private const val MSG_INVALID_JUBULA_HOME = "specified $envJubulaHome is not a valid directory"
    ...
    fun deriveCommandLine(args: Array<String>): CommandLine {
        ...
        formatter.width = 115
        formatter.optionComparator = compareBy { it.description }
        val cmd = CliUtils.getCommandLine("$scriptName$programExt", args, cliOptions, formatter)
        if (cmd == null) exit("unable to proceed... exiting", RC_BAD_CLI_ARGS)
        return cmd
    }
    ...
    fun main(args: Array<String>) {
        ...
        if (!FileUtil.isDirectoryReadable(location)) exit(MSG_INVALID_APP_LOCATION + location, RC_BAD_CLI_ARGS)

        val exe = cmd.getOptionValue("e")
        val exeFqn = StringUtils.appendIfMissing(location, separator) + exe
        if (!FileUtil.isFileExecutable(exeFqn)) exit(MSG_INVALID_APP + exeFqn, RC_BAD_CLI_ARGS)

        val exeArgs = RuntimeUtils.formatCommandLine(cmd.getOptionValue("p"))

        // 2. resolve optional cli options
        val jubulaHome = cmd.getOptionValue("h", System.getenv(envJubulaHome))
        if (StringUtils.isBlank(jubulaHome)) exit(MSG_MISSING_JUBULA_HOME, RC_BAD_CLI_ARGS)
        if (!FileUtil.isDirectoryReadable(jubulaHome)) exit(MSG_INVALID_JUBULA_HOME, RC_BAD_CLI_ARGS)
    ...
    private fun handleAutAgent(agentOption: String) {
        ...
        if (agentConfigs.size != 2) exit(MSG_INVALID_AGENT_CONFIG + agentOption, RC_BAD_CLI_ARGS)

        val agentHost = agentConfigs[0]
        val agentPort = agentConfigs[1]
        if (!NumberUtils.isDigits(agentPort)) exit(MSG_INVALID_AGENT_PORT + agentPort, RC_BAD_CLI_ARGS)

        val port = NumberUtils.toInt(agentPort)
        if (port < 1024) exit(MSG_INVALID_AGENT_PORT + agentPort, RC_BAD_CLI_ARGS)
    ...
    private fun exit(message: String, exitCode: Int) {
