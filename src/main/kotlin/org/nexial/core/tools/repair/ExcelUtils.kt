...
object ExcelUtils {
	...
	private fun copySheet(
		sourceSheet: XSSFSheet, targetWorkbook: XSSFWorkbook, lastDataRow: Int, fileType: ArtifactType,
	): XSSFSheet? {
	...
	private fun copyRow(sourceRow: XSSFRow, targetRow: XSSFRow, fileType: ArtifactType = DATA) {
		...
		for (columnIndex in 0 until lastColumn + 1) {
			// set column width for one row only; it's enough
			...
			val targetCell = targetRow.getCell(columnIndex) ?: targetRow.createCell(columnIndex, sourceCell.cellTypeEnum)
