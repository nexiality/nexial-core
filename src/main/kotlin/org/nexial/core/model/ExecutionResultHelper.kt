...
class ExecutionResultHelper(
	...
	private fun formatDescriptionCell(sheet: XSSFSheet, startRow: Int, prefix: String, lastRow: Int): Int {
		...
		when ("$target.$command") {
			CMD_REPEAT_UNTIL -> {
				val indent =
					if (StringUtils.containsAny(prefix1, REPEAT_DESCRIPTION_PREFIX, SECTION_DESCRIPTION_PREFIX))
						"  "
					else
						""
				prefix1 +=
					if (StringUtils.contains(prefix1, REPEAT_DESCRIPTION_PREFIX))
						""
					else
				...
				var numOfSteps = Integer.parseInt(Excel.getCellValue(row.getCell(COL_IDX_PARAMS_START)))
				var j = 0
				while (true) {
			...
			CMD_SECTION      -> {
				val indent = if (StringUtils.containsAny(prefix1,
				                                         REPEAT_DESCRIPTION_PREFIX,
				...
				ExcelStyleHelper.formatDescriptionCell(row.getCell(COL_IDX_DESCRIPTION), prefix1)
				prefix1 = indent + prefix1
				var numOfSteps = NumberUtils.toInt(Excel.readCellValue(row, COL_IDX_PARAMS_START))
				var j = 0
				while (true) {
					if (j == numOfSteps) break
					val num = formatDescriptionCell(sheet, startRow + j + 1, prefix1, lastRow)
	...
	private fun writeTestResults(
		...
		if (CollectionUtils.isNotEmpty(macroSteps)) {
			...
			for (j in 0 until stepSize) {
				...
				if (sectionEndIndex != -1 && j > sectionEndIndex) {
					sectionEndIndex = -1
				}
	...
	private fun handleNestedMessages(
		...
		if (testStep.commandFQN == CMD_VERBOSE || CollectionUtils.isEmpty(nestedTestResults))
			return Pair(currentRow, lastRow)
