...
public class SysdateManualTest {
	...
	public void testFirstDOW() {
		...
		Assert.assertEquals("009", sysdate.firstDOW("ddd"));
	}

	@Test
	public void testLastDOW() {
		// manual update needed
		Assert.assertEquals("15", sysdate.lastDOW("dd"));
	}

	@Test
	public void testLastQtr() {
		Assert.assertEquals("2018-2", sysdate.lastQtr("yyyy-q"));
		Assert.assertEquals("2018-2", sysdate.lastQtr("yyyy-qqq"));
		Assert.assertEquals("2", sysdate.lastQtr("qqq"));
	}

	@Test
	public void testCurrentQtr() {
		Assert.assertEquals("2018-3", sysdate.currentQtr("yyyy-q"));
		Assert.assertEquals("2018-3", sysdate.currentQtr("yyyy-qqq"));
		Assert.assertEquals("3", sysdate.currentQtr("qqq"));
	}

	@Test
	public void testNextQtr() {
		Assert.assertEquals("2018-4", sysdate.nextQtr("yyyy-q"));
		Assert.assertEquals("2018-4", sysdate.nextQtr("yyyy-qqq"));
